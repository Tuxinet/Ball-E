...
bloom_model = AutoModelForCausalLM.from_pretrained(model_name, device_map=device_map, offload_folder="offload", offload_state_dict = True, torch_dtype=torch.float16, load_in_8bit=True, quantization_config=quantization_config)
bloom_tokenizer = AutoTokenizer.from_pretrained(model_name, use_fast = False)
...
---""".format(name)
    ...
    async for m in message.channel.history(limit=50, oldest_first=True, around=datetime.datetime.now()):
